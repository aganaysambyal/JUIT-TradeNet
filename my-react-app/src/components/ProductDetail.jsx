...
function ProductDetail() {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      socket.emit('joinProductRoom', productId); // Join the room
    });

    socket.on('getMsg', (data) => {
      setmsgs(data); // Scoped messages
    });
  ...
  const handleSend = () => {
    const data = {
      ...
      msg,
      productId, // Send the productId with each message
    };
  ...
  const handleDeleteChat = () => {
    socket.emit('deleteAllMsgs', productId); // Emit event to delete all messages
  };
          ...
          <>
            ...
            <div className="image-container">
              <img
                ...
                onClick={() => setSelectedImage(`http://localhost:4000/uploads/${product.pimage}`)}
              />
              {product.pimage2 ? (
                <img
                  src={`http://localhost:4000/uploads/${product.pimage2}`}
                  alt={product.pname}
                  className="product-image2"
                  onClick={() => setSelectedImage(`http://localhost:4000/uploads/${product.pimage2}`)}
                />
              ) : (
