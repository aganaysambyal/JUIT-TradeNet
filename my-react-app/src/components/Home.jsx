...
function Home() {
  ...
  useEffect(() => {
    ...
    axios.post("http://localhost:4000/liked-products", { userId })
      .then((res) => {
        if (res.data.products) {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      if (search.trim() === '') {
        ...
        axios.get(`http://localhost:4000/search-products?query=${search}`)
          .then((res) => {
            setcProducts(res.data.products);
            setHasSearched(true);
          })
  ...
  const handleLike = (productId, e) => {
    ...
    axios.post(`http://localhost:4000/${endpoint}`, { userId, productId })
      .then((res) => {
          ...
          cproducts.length > 0 ? (
            cproducts.map((item) => (
              <div className="card" key={item._id} onClick={() => handleProduct(item._id)}>
                ...
                <img className="product-image" src={'http://localhost:4000/uploads/' + item.pimage} alt="Product" />
                <p>{item.pname}</p>
